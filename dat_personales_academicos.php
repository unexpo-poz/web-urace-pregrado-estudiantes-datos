...
<!-- PARA GENERAR LOS ESTADOS, CIUDADES, MUNICIPIOS Y PARROQUIA DE DIRECCION PERMANENTE-->
...
<title>DATOS PERSONALES Y ACADEMICOS</title>
		...
		$ciudad=$dirp_e[0];
		$estado=$dirp_e[1];//PREGUNTAR:recordar, se debe incluir el municipio y la parroquia//
		$tot_hab_n=$dirp_e[2];//inclui esta variable que se va a utilizar para el municipio//
		$edo_nac_e=$dirp_e[3];// inclui esta variable que se va a utilizar para la parroquia//
		$avenida=$dirp_e[4];
		$urbanizacion=$dirp_e[5];
		$manzana=$dirp_e[6];
		$edificio=$dirp_e[7];
		$nrocasa=$dirp_e[8];
		//////////////////////////////////////////
		...
		$tot_raz_v=$conex->result[0][19];//es el campo donde se guardara la parroquia de nacimiento
		$depend_p_e=$conex->result[0][25];//es el campo donde se guardara el municipio de nacimiento
		...
		$ciudad=$conex->result[0][124];//CIUDAD donde vive el estudiante (Tabla:DACE002)
		$estado=$conex->result[0][125];//ESTADO donde vive el estudiante (Tabla:DACE002)
		$tot_hab_n=$conex->result[0][20];//MUNICIPIO - Variable que se va a tomar para guardar el municipio (Tabla:DACE002)
		$edo_nac_e=$conex->result[0][100];//PARROQUIA - Variable que se va tomar para guardar la parroquia (Tabla:DACE002)
		...
		if ($filas==1)
		{
			...
			$sqlACADEMICO.= "PROMEDIO, COSTO_MENSUAL, CODIGO_P, CODIGO_E, CODIGO_C, CODIGO_M, ANO_EGRE_COLE, CODIGO_PQUIA ";
			...
			$codigo_pquia = $conex->result[0][12];
 	...
 	<body onload="javascript:mostrar(); calcularEdad(); conf_inicial_NAC(); ciudadesr(estado_d.value,ciudad_d.value);
	municipios_res(estado_d.value,municipio_d.value);	parroquia_res(estado_d.value,municipio_d.value,parroquia_d.value);
	municipios_naci(edo_nacimiento.value,pais_hidden.value,munic_nac.value); parroquia_naci(edo_nacimiento.value,munic_nac.value,parroq_nac.value); parroquia_plantel(codedoplantel.value,codmpioplantel.value,parroqplantel.value);">
...
INFORMACION PERSONAL Y ACADEMICA
	...
	<input type="hidden" name="munic_nac" id="munic_nac" value="<?php echo $depend_p_e; ?>" />
	<input type="hidden" name="parroq_nac" id="parroq_nac" value="<?php echo $tot_raz_v; ?>" />
	...
	<input type="hidden" name="estado_d" id="estado_d" value="<?php echo $estado; ?>" />
	<input type="hidden" name="ciudad_d" id="ciudad_d" value="<?php echo $ciudad; ?>" />
	<input type="hidden" name="municipio_d" id="municipio_d" value="<?php echo $tot_hab_n; ?>" />
	<input type="hidden" name=parroquia_d" id="parroquia_d"value="<?php echo $edo_nac_e; ?>" />
	...
	<input type="hidden" name="parroqplantel" id="parroqplantel" value="<?php echo $codigo_pquia; ?>" />
...
<tr>
    ...
    <option value="12"> Diciembre</option>
     ...
     <select name="ano" class="datospf" id="ano" onchange="calcularEdad();"  readonly="readonly" style="font-size:10px;">
...
<?php */?> -->     
	...
	<input type="hidden" name="hidden_muninaci" value="<?php echo $codigo_m; ?>">
    <td>
	...
	<select name="edo_nac_e" id="edo_nac_e" class="datospf" onChange="ciudades(this.value,pais_nac_e.value);municipios_naci(this.value,pais_nac_e.value,hidden_muninaci.value);">
			<option selected="selected" value=0> -SELECCIONE- </option>
	<?php
			for ($e = 0; $e < $filas2; $e++){
	...
	<tr>
	  <td colspan="1">
	 <span id="mpio_nac" style=""> 
	  <font color= NAVY>Municipio de Nacimiento:</font><br/>
	  <div id="capa_mn">
	    <select name="municipio_nac" id="municipio_nac">
		   <option value="">-SELECCIONE-</option>
	    </select>
	  </div>
	 </span>
   ...
   <td colspan="1">
	<span id="parq_nac" style=""> 
	<font color= NAVY>Parroquia de Nacimiento:</font><br/>
     <div id="capa_pn">
	  <select name="parroquia_nac" id="parroquia_nac">
		<option value="">-SELECCIONE-</option>
	  </select>
	 </div>
	</span>
...
<tr>
    ...
    <font color= NAVY><B>Direcci&oacute;n Permanente:</B></font></font>	
     ...
     <!-- <td colspan="1"><font color= NAVY>Pais:</font><br/>
	<select name="pais" id="pais" class="datospf" onChange="paisdir(this.value);">
		<option selected="selected" value=0>-SELECCIONE-</option>
		<?php
		/*for ($p=0; $p<$filas1; $p++){
			$CODIGO = $conexPais[$p][0];
...
-->
<tr>
    <input type="hidden" name="hidden_muni_res" value="<?php echo $tot_hab_n; ?>"><!--VALOR DE LA BD (MUNICIPIO DE RESIDENCIA,PARA OBTENER SU VALOR Y PASARLO A LA FUNCION Y SE MUESTRE CUANDO SE GUARDE EL SELECT)-->  
	<input type="hidden" name="hidden_cd_res" value="<?php echo $ciudad; ?>"><!--VALOR DE LA BD(CIUDAD DE RESIDENCIA)-->
    <td colspan="1"><font color= NAVY>Estado:</font><br/>
   	  <span id="edo_dir" style=""> 
	   	<select name="estado" id="estado" onChange="ciudadesr(this.value, hidden_cd_res.value); municipios_res(this.value, hidden_muni_res.value);"> 
		  <option selected="selected" value=0> -SELECCIONE- </option>
		<?php
		   for ($e = 0; $e < $filas2; $e++)
		   {
			  $CODIGO = $conex_estado[$e][0];
			  $EDO_NOMBRE = $conex_estado[$e][1];
			  if ($estado == $CODIGO) { //valor de la BD
				?>
				<option value="<?php echo $CODIGO; ?>" selected><?php echo $EDO_NOMBRE; ?></option>
				<?php
			   }
			   else {
				?>
   ...
   <td colspan="1"> 
    <span id="cd_dir" style=""> 
	 <label for="ciudad_d">
      <font id="ciudad_d" color= NAVY>Ciudad:</font><br/>
       <div id="capa_r">
   ...
   <td colspan="1">
	 <span id="mpio_dir" style=""> 
	  <font color= NAVY>Municipio:</font><br/>
	  <div id="capa_m">
  ...
  <td colspan="1">
	<span id="parq_dir" style=""> 
	<font color= NAVY>Parroquia:</font><br/>
     <div id="capa_pa">
...
<tr>
	...
	<td colspan="1">
	<input type="hidden" name="hidden_muniplantel" value="<?php echo $depend_p_e; ?>">  
	...
	<select name="edo_plantel" id="edo_plantel" class="datospf" onChange="ciudades_p(this.value,pais_plantel.value); ;
					municipios(this.value,pais_plantel.value,hidden_muniplantel.value);">
		<option selected="selected" value=0> -SELECCIONE- </option>
		<?php
		for ($e = 0; $e < $filas2; $e++){
			$CODIGO = $conex_estado[$e][0];
...
<tr>
  ...
  <td colspan="1">
	<span id="parroq_plantel" style="display:none"> 
	<font color= NAVY>Parroquia de Ubicaci&oacute;n del Plantel:</font><br/>
     <div id="capa_pl">
	  <select name="parroquia_plant" id="parroquia_plant" style="display:none">
		<option value="">-SELECCIONE-</option>
	  </select>
	 </div>
	</span>
